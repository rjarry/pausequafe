...
public class TreeSortFilterProxyModel extends QSortFilterProxyModel {
	...
	public void setSortMode(int sortMode) {
		...
		setSortRole(sortMode); // sort role has to be change in order to trigger
								// the sorting
	...
	protected boolean lessThan(QModelIndex left, QModelIndex right) {
		...
		case SORT_BY_META_LEVEL:
			if (leftElement instanceof ItemElement && rightElement instanceof ItemElement) {
				...
				// equal
				result = (leftElement.getItem().getMetaLevel() < rightElement.getItem()
						.getMetaLevel())
						|| (leftElement.getItem().getMetaLevel() == rightElement.getItem()
								.getMetaLevel() && leftElement.getName().compareTo(
								rightElement.getName()) < 0);
			} else {
		...
		case SORT_BY_REMAINING_TRAINING_TIME:
			if (leftElement instanceof ItemElement && rightElement instanceof ItemElement) {
				result = leftElement.getItem().getRemainingTrainingTime(sourceModel.getSheet()) < rightElement
						.getItem().getRemainingTrainingTime(sourceModel.getSheet());
			} else {
				result = leftElement.getName().compareTo(rightElement.getName()) < 0;
			}
			break;
